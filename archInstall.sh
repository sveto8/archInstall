...
if [[ "${DE_CHOICE}" == "3" ]]; then
    # Ovdje koristimo $NEW_USERNAME iz roditelja – ne escapat!
    HYPR_USER="${NEW_USERNAME:-root}"

    # SVE ostale reference na HYPR_USER i HYPR_HOME moraju biti escaped:
    if ! id -u "\${HYPR_USER}" >/dev/null 2>&1; then
        echo "User \${HYPR_USER} not found in chroot. Skipping Hyprland config."
    else
        # getent se izvršava unutar chroot-a – escapat cijelu naredbu
        HYPR_HOME="\$(getent passwd \"\${HYPR_USER}\" | cut -d: -f6)"
        [[ -n "\${HYPR_HOME}" ]] || HYPR_HOME="/root"

        # Stvori direktorije (escapane varijable)
        ...
        # hyprland.lua (citirani heredoc – ne treba escaping)
...
echo "6. Copy setupAfterInstall.sh to the new system"
