...
Wipes and partitions the target disk, sets up LUKS2 + Btrfs, installs the
base system, and leaves the machine in a bootable (but not yet fully
configured) state.
...
8. `pacstrap`s the base system (+ microcode package auto-detected from
   CPU vendor, + chosen DE packages)
9. Creates the regular user and sets its password, then sets the root
   password — all *before* the long unattended steps, so you can walk
   away once passwords are set
10. Unattended: hostname, timezone, locale (`en_US.UTF-8` +
    `hr_HR.UTF-8` generated by default), keymap, `/etc/hosts`, minimal
    `mkinitcpio.conf` (systemd + `sd-encrypt`, no Plymouth yet), GRUB
    install + config, display manager enabled if a DE was chosen
